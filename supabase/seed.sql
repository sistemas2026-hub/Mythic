...
-- Productos (perfumes) — familia "Perfumes", vendibles en el POS
insert into public.products (name, brand_id, category_id, family_id, gender, concentration, volume_ml, barcode, sku, price, cost)
select p.name, b.id, c.id,
       (select id from public.product_families where slug = 'perfumes'),
       p.gender, p.concentration, p.volume_ml, p.barcode, p.sku, p.price, p.cost
from (values
...
-- Inventario inicial en la sucursal Centro (cantidades del mockup + insumos)
